...
(() => {
  ...
  const portalControls = () => {
    ...
    credits.className = 'song-portal-credits';
    credits.textContent = String(window.VCardUI?.finTit || '');
    credits.hidden = true;
    controls.append(credits);
    state.credits = credits;
    const fitCredits = () => {
      if (credits.hidden || !controls.clientWidth) return;
      credits.style.fontSize = `${controls.clientWidth / 10}px`;
      if (credits.scrollWidth > credits.clientWidth) {
        const size = parseFloat(credits.style.fontSize);
        credits.style.fontSize = `${size * credits.clientWidth / credits.scrollWidth}px`;
      }
    };
    state.fitCredits = fitCredits;
    new ResizeObserver(fitCredits).observe(controls);
  ...
  const setPhase = (phase) => {
    ...
    const finishing = phase === 'finish' && sharedSongAudio.ended;
    const creditsVisible = finishing && ['credits-in', 'credits-hold'].includes(state.controls?.dataset.ending);
    if (state.credits) {
      state.credits.hidden = !creditsVisible;
      if (creditsVisible) requestAnimationFrame(state.fitCredits);
    }
    ...
    if (state.action) {
      ...
      state.action.classList.toggle('is-visible', !isActuallyPlaying && !finishing);
  ...
  const showTape = ({
    ...
    loadFinishMedia().then((media) => {
      ...
      const shouldPlay = (
        play
        && !(phase === 'finish' && ['end-fade', 'credits-in', 'credits-hold'].includes(state.controls?.dataset.ending))
        && portalMotionAllowed
        && (
          allowPausedPlayback
          || allowEndedPlayback
  ...
  const scheduleTrackEndSequence = () => {
    // Credits begin only when the audio has ended, never over its final seconds.
    if (sharedSongAudio.ended) {
  ...
  document.addEventListener('vcard:silent-playback-phase', (event) => {
    ...
    if (detail.phase === 'start-hold') {
      ...
      state.controls.dataset.ending = 'cassette';
      showTape({ play: true, reset: true, phase: 'finish', allowEndedPlayback: true });
    } else if (['end-fade', 'credits-in', 'credits-hold'].includes(detail.phase)) {
      portalVideo().pause();
      state.controls.dataset.ending = detail.phase;
      setPhase('finish');
    }
  });
})();
...
(() => {
  ...
  (() => {
    ...
    sitemapButton.className = 'vcard-player-sitemap';
    ...
    const SILENT_ANI_STORAGE_KEY = 'vcard-silent-ani';
    const normalizeSilentAni = (value) => (
      ['0', '3', '5', '10'].includes(String(value)) ? String(value) : '5'
    );
    let silentAni = normalizeSilentAni(localStorage.getItem(SILENT_ANI_STORAGE_KEY));
    ...
    const startSilentPhase = (
      ...
      phase,
      onComplete,
      durationMs = SILENT_PHASE_DURATION_MS
    ) => {
      ...
      document.dispatchEvent(new CustomEvent('vcard:silent-playback-phase', {
        detail: { preview, phase, durationMs }
      }));
      // Silence animates only the portal. Time and seek position belong to audio.
    ...
    const startPlaybackWithIntro = (preview, onComplete) => {
      const durationMs = Number(silentAni) * 1000;
      if (!durationMs) {
        onComplete();
        return;
      }
      startSilentPhase(preview, 'start-rewind', () => {
        stopSilentPhase({ restore: false });
        onComplete();
      }, durationMs);
    };
    ...
    const bindAutoAdvance = (audioEl, currentButton) => {
      ...
      audioEl.onended = () => {
        ...
        const phases = [
          ['end-rewind', Number(silentAni) * 1000],
          ['end-fade', 3000],
          ['credits-in', 3000],
          ['credits-hold', 5000],
        ];
        const advancePhase = (index) => {
          if (
            !audioEl.ended
          ...
          if (index < phases.length) {
            const [phase, durationMs] = phases[index];
            if (!durationMs) {
              advancePhase(index + 1);
              return;
            }
            startSilentPhase(endedPreview, phase, () => advancePhase(index + 1), durationMs);
            return;
          }
          stopSilentPhase({ restore: false });
          if (songAlternation === 'none') return;
          const continuePlaylist = () => {
            if (!audioEl.ended || playingPreview !== endedPreview
                || (audioEl.currentSrc || audioEl.src) !== endedSource) return;
            stopSilentPhase({ restore: false });
            if (songAlternation === 'none') return;
          ...
          if (Number(silentAni)) {
            startSilentPhase(endedPreview, 'between-songs', continuePlaylist, Number(silentAni) * 1000);
          } else continuePlaylist();
        };
        advancePhase(0);
      };
    };
    ...
    const playSong = (preview, button, { includeStaticHold = false } = {}) => {
      ...
      const startFromBeginning = () => {
        ...
        audioEl.play().catch((error) => {
          if (playingPreview !== preview || audioEl.getAttribute('src') !== audioSrc) return;
          stopSilentPhase();
      ...
      document.dispatchEvent(new CustomEvent('vcard:prepare-audio-context'));
      // With preload=none metadata may arrive only after play(). Waiting for it
      // here can deadlock startup and leave an uncancellable pending listener.
      startPlaybackWithIntro(preview, () => {
    ...
    (() => {
      ...
      const applySetting = (key, value) => {
        ...
        if (key === 'background') {
          ...
          cancelAutoAdvance();
        } else if (key === 'silentAni') {
          silentAni = normalizeSilentAni(value);
          state[key] = silentAni;
          localStorage.setItem(SILENT_ANI_STORAGE_KEY, silentAni);
